...
class order_information:
    ...
    def __init__(self, name, oa):
        ...
        self.o_id = 0
        self.o_price = 0  # オーダー価格(Marketの場合、現在Mid価格）　Planで代用可？
        self.o_time = 0
        self.o_state = 0
        self.o_time_past = 0
        self.o_tp_range = 0  # Planで代用可？
        self.o_tp_price = 0  # Planで代用可？
        self.o_lc_range = 0  # Planで代用可？
        self.o_lc_price = 0  # Planで代用可？
        self.o_units = 0  # Planで代用可？
        self.o_direction = 0  # Planで代用可？
        # トレード情報
        self.t_id = 0
    ...
    def print_info(self):
        ...
        print("   【order1】", self.o_id, self.o_price, self.o_time, self.o_state, self.o_time_past)
        print("   【order2】", self.o_tp_range, self.o_tp_price, self.o_lc_range, self.o_lc_price, self.o_units, self.o_direction)
        print("   【trade1】", self.t_id, self.t_type, self.t_initial_units, self.t_current_units, self.t_time, self.t_time_past)
        print("   【trade2】", self.t_price, self.t_state, self.t_realize_pl, self.t_pips, self.t_close_time, self.t_close_price)
    ...
    def make_order(self):
        ...
        if order_ans['cancel']:  # キャンセルされている場合は、リセットする
            tk.line_send(" 　Order不成立（今後ループの可能性）", self.name, order_ans['order_id'],f.now())
            return 0  # 情報の登録は出来ないはず

        self.o_id = order_ans['order_id']
        self.o_price = order_ans['price']  # オーダー価格(Marketの場合、現在Mid価格）　Planで代用可？
        self.o_time = order_ans['order_time']
        self.o_state = "PENDING"  # 初回はPENDINGを必ず入れる（状態変更把握の為）
        self.o_time_past = 0  # 初回は変更なし
        self.o_tp_range = order_ans['tp_range']  # Planで代用可？
        self.o_tp_price = float(order_ans['tp_price'])  # Planで代用可？
        self.o_lc_range = order_ans['lc_range']  # Planで代用可？
        self.o_lc_price = float(order_ans['lc_price'])  # Planで代用可？
        self.o_units = float(order_ans['unit'])  # Planで代用可？
        self.o_direction = order_ans['direction']  # Planで代用可？
    ...
    def update_information(self):  # orderとpositionを両方更新する
        ...
        if self.order['state'] == "PENDING" and latest_info['order_state'] == 'FILLED':  # オーダー達成（Pending⇒Filled）
            if latest_info['position_state'] == 'OPEN':  # ポジション所持状態
                ...
                tk.line_send("  (取得)", self.name, order_information.position_times, "個目",
